...
class TableParserSpec extends WordSpec with MustMatchers {
  ...
  "The parser" should {
    "parse one table" in {
      ...
      table.columns mustEqual Seq(Column("P_PARTKEY", 4), Column("P_NAME", 56))
    }
    ...
    "parse multiple tables" in {
      ...
      res.length mustBe 2
      res mustEqual Seq(
        Table("PART", Seq(Column("P_PARTKEY", 4), Column("P_NAME", 56))),
        Table("ORDER", Seq(Column("O_ORDERKEY", 12), Column("O_AMOUNT", 11)))
      )
    }
  }
}
