...
nation, region where
p_partkey = ps_partkey
and s_suppkey = ps_suppkey and s_nationkey = n_nationkey and n_regionkey = r_regionkey and r_name = '[REGION]'
) order by
...
from
supplier,
lineitem, orders, customer, nation as n1, nation as n2
where
s_suppkey = l_suppkey
...
from
part,
supplier, lineitem, orders, customer, nation as n1, nation as n2, region
where
p_partkey = l_partkey
...
and o_orderdate between date '1995-01-01' and date '1996-12-31' and p_type = '[TYPE]'
...
from (
select
c_custkey, count(o_orderkey) from
...
p_partkey = l_partkey
and p_brand = '[BRAND1]'
and p_container in ( 'SM CASE', 'SM BOX', 'SM PACK', 'SM PKG') and l_quantity >= [QUANTITY1] and l_quantity <= [QUANTITY1] + 10 and p_size between 1 and 5
and l_shipmode in ('AIR', 'AIR REG')
and l_shipinstruct = 'DELIVER IN PERSON'
) or (
p_partkey = l_partkey
and p_brand = '[BRAND2]'
and p_container in ('MED BAG', 'MED BOX', 'MED PKG', 'MED PACK') and l_quantity >= [QUANTITY2] and l_quantity <= [QUANTITY2] + 10 and p_size between 1 and 10
and l_shipmode in ('AIR', 'AIR REG')
and l_shipinstruct = 'DELIVER IN PERSON'
) or
(
p_partkey = l_partkey
and p_brand = '[BRAND3]'
and p_container in ( 'LG CASE', 'LG BOX', 'LG PACK', 'LG PKG') and l_quantity >= [QUANTITY3] and l_quantity <= [QUANTITY3] + 10 and p_size between 1 and 15
and l_shipmode in ('AIR', 'AIR REG')
and l_shipinstruct = 'DELIVER IN PERSON'
);
...
supplier, lineitem as l1, orders, nation
where
...
select
c_phone as cntrycode, c_acctbal
from
customer
where
c_phone in
...
where
c_acctbal > 0.00
and c_phone in ('[I1]','[I2]','[I3]','[I4]','[I5]','[I6]','[I7]')
)
and not exists (
select
*
