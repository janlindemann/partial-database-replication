...
import de.hpi.epic.partialTableDistribution.utils.{Query, TreeCombinations}
...
/**
  ...
  */
object SingleBucket {
  private def weight(s: Seq[Query]): Long = s.flatMap(_.columns).distinct.aggregate(0l)(_ + _.size, _ + _)
